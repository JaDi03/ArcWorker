...
const navigation = [
    ...
    { name: 'Review', href: '/business/review' },
];
...
export default function BusinessLayout({ children }: { children: React.ReactNode }) {
    ...
    // Check for username registration for Agencies too
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (mounted && status === 'disconnected' && !isConnected && !address && !hasLocalSession) {
            console.log("AgencyLayout: Redirecting to home - Reason: Disconnected and no session.");
            router.push('/');
        }
                ...
                <div className="p-4 border-t border-white/10">
                    ...
                    >
                        ...
                        <div className="overflow-hidden">
                            <p className="text-xs font-bold text-white truncate">{profile?.name || 'Business Portal'}</p>
