...
export const WorkerTaskFeed: React.FC<WorkerTaskFeedProps> = ({ onBack }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            localStorage.setItem('arc_submitted_tasks', JSON.stringify(Array.from(recentlySubmitted)));
        }
    }, [recentlySubmitted]);
    ...
    const availableTasks: TaskOpportunity[] = useMemo(() => {
        ...
        if (currentUserLower) {
            rawTasks.forEach((t: any) => {
                ...
                if (isParticipated) {
                    ...
                    // Ensure the groupKey itself is in the set for future stability
                    if (!recentlySubmitted.has(groupKey)) {
                        setRecentlySubmitted(prev => new Set(prev).add(groupKey));
                    }
                }
            });
        }
        ...
        const isAvailable = (t.status === 0 || t.status === 1) &&
            ...
            !recentlySubmitted.has(groupKey) && // NEW: Check group block
            !t.hasParticipated;
    ...
    try {
        ...
        if (isConnected) {
            ...
            setRecentlySubmitted(prev => {
                ...
                console.log(`[Worker Feed] Marking participated (EOA): ID=${selectedTask.id}, Group=${selectedTask.groupKey}`);
                return next;
            });
