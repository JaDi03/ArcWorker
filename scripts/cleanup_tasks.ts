...
const loadEnv = (filename: string) => {
    try {
        const envPath = path.resolve(__dirname, '../frontend', filename);
...
if (!PRIVATE_KEY) {
    console.error("FATAL: DEPLOYER_PRIVATE_KEY not found in ../frontend/.env.local or .env");
    process.exit(1);
}
...
async function cleanupTasks() {
    ...
    if (args.length === 0) {
        console.log("Usage: npx tsx scripts/cleanup_tasks.ts <TASK_ID_1> <TASK_ID_2> ...");
        process.exit(0);
    }
